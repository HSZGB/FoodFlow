...
\end{titlepage}
    ...
    \subsection{推荐系统模块：序列、SPU 与 KG 增强}
    ...
    \textbf{KG-Tripartite（知识图谱增强三方推荐）：}在三方重排架构之上，引入 \texttt{foodflow/recommenders.py} 中实现的轻量知识图谱兴趣信号。该模型不训练完整图神经网络，而是将 TRD 结构化字段（商家类目、商圈区域、价格段、评分等级、口味、食材、品牌等）组织为知识图谱三元组，通过时间衰减将用户历史订单转化为图谱上的带权兴趣边，在候选排序中叠加图谱路径匹配特征。KG-Tripartite 作为 FoodFlow 主推荐链路的一部分，通过时间衰减与关系加权匹配将图谱信号接入三方重排，并可从历史下单、品类、区域和价格段构造路径证据解释推荐原因。

    \subsection{三方重排模块：用户、商家与履约约束}
    ...
    \subsection{KG 增强与可解释推荐模块}

    FoodFlow 的轻量 KG 模块（\texttt{foodflow/kg.py}）与主推荐器（\texttt{foodflow/recommenders.py}）配合工作：前者将现有结构化字段转化为知识图谱三元组和可解释推荐路径，后者在 KG-Tripartite 中把图谱兴趣信号作为三方重排的增强分量。该模块不单独作为外部子项目呈现，而是融入 FoodFlow 的推荐系统主链路。
    ...
    \subsubsection{图谱兴趣构造：时间衰减与关系匹配}
    ...
    下单前点击序列反映当前一餐的短期偏好。设点击序列为 $\mathcal{C}_{u,T}=(c_1,c_2,\ldots,c_m)$，越靠近下单的点击权重越高：$w^{click}_{u,c_j,T}=\eta \rho^{m-j}$（$\eta=1.6,\ \rho=0.88$）。点击序列生成的临时偏好为 $p^{click}_{u,e}=\sum_{c_j\in \mathcal{C}_{u,T}} w^{click}_{u,c_j,T} A_{c_j,e}$。KG-Tripartite 将历史偏好与短期点击偏好合并为轻量图谱兴趣分，并与用户分、商家公平分、ETA 分和供给分共同进入三方重排。
    ...
    \section{系统评估：推荐效果与履约表现}

    \subsection{FoodFlow 系统闭环实验}

    FoodFlow 主系统在完整 TRD 训练集（1,068,495 条订单）上进行离线推荐评估，并使用 10 个随机种子的动态履约仿真进行系统级评价。以下分别报告推荐侧、履约侧和系统权衡分析。

    \subsubsection{离线推荐结果}
    ...
    \begin{enumerate}
        ...
        \item \textbf{KG-Tripartite 在三方模型中准确率最高：}Recall@20 达到 0.4263，NDCG@20 达到 0.3480，Exposure Gini 低至 0.8746，说明知识图谱引入的品类/商圈/价位结构化信号在保持品类校准的同时有效提升了排序精度。
        \item \textbf{Logistic-LTR 在覆盖面上表现最佳：}Coverage@20 达到 0.4290，Exposure Gini 最低（0.8282），表明学习排序方法能有效降低曝光集中度。注意当前环境使用 Logistic-LTR 作为学习排序后备方案，完整 LightGBM-LTR 在 Coverage@20（0.4345）和 Exposure Gini（0.7942）上表现更优。
    ...
    从图 \ref{fig:tradeoff} 可以看到，Logistic-LTR 位于准确率-公平性前沿的右下角（较高准确率、较低曝光集中度），Seq-Tuned 则位于高准确率但曝光略集中的位置，三方模型在保持较高准确率的同时维持了适中的公平性。

    \subsubsection{动态履约仿真结果（常规运力 120 骑手）}
    ...
    \subsubsection{运力紧张压力场景（30 骑手高峰爆单）}
    ...
    \begin{itemize}
        \item \textbf{离线准确率前沿：}Seq-Tuned 和 Logistic-LTR 代表推荐准确率的上界，在 Recall@20 上分别达到 0.4675 和 0.4013（完整 LightGBM-LTR 可达到 0.4424）。
        \item \textbf{系统效用前沿（常规运力 120 骑手）：}Session-SPU-Tripartite + Batch、Seq-xQuAD-Tripartite + Batch 和 KG-Tripartite + Batch 代表平台效用的上界，分别达到 0.557、0.553 和 0.550。
        \item \textbf{系统效用前沿（运力紧张 30 骑手）：}KG-Tripartite + RouteMinETA 在压力场景下平台效用达到 0.544，远超同场景下最优槽位匹配的 0.486，说明路径感知顺路派单是高峰爆单时的韧性机制。
    \end{itemize}

    这三类策略共同构成了三方推荐的权衡边界——说明在外卖推荐场景中，纯粹的准确率优化和系统级多目标优化并不完全重合，需要根据平台的战略偏好进行折中。图 \ref{fig:scorecard} 的综合记分卡进一步印证了这一发现：三方模型在配送时效和平台效用上表现优异，而 Seq-Tuned 在用户侧指标上保持领先。

    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.85\linewidth]{demo.png}
        \caption{FoodFlow Streamlit 交互演示界面——展示推荐工作台、空间分布与高峰回放}
        \label{fig:demo}
    \end{figure}
    ...
    \begin{enumerate}
        ...
        \item \textbf{知识图谱信号能增强三方推荐的排序精度与可解释性。}KG-Tripartite 在三方模型中离线准确率最高（Recall@20=0.4263、NDCG@20=0.3480），同时通过品类/商圈/价位结构化路径提供可解释推荐证据。该能力已经融入 \texttt{foodflow/recommenders.py} 的 KG-Tripartite 主链路，而不是作为独立模块单独评估。
    ...
    \begin{itemize}
        ...
        \item KG-Tripartite 当前使用的是免训练的轻量图谱近似（时间衰减 + 关系加权匹配），尚未引入端到端图神经网络训练；后续可在现有 \texttt{foodflow/recommenders.py} 主链路内继续增强图谱表示学习。
        \item 当前三方优化采用"推荐重排 + 下单后骑手匹配"的解耦闭环，不是工业级端到端联合调度系统。
    \end{itemize}

    总体而言，FoodFlow 验证了"推荐 + 履约联合优化"这一技术路线的可行性。后续工作可以围绕更精细的骑手行为建模（如接单意愿学习、多平台竞争）、KG 表示学习与三方重排的端到端融合、更大规模的真实数据验证以及在线 A/B 测试展开。
