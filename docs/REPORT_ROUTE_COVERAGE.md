...
当前仓库已经按“报告B工程闭环为主、报告A轻量增强为辅”的路线完成核心交付：TRD 数据处理、默认七模型离线评估、Repeat/Explore 分段指标、七条动态履约仿真、LightGBM/Logistic 学习排序、训练期 Session/SPU 增强、MNL 用户选择、批量骑手匹配、骑手参数校准、轻量 KG 解释、三方分量归一化、Pareto 前沿、报告和 NotebookLM PPT 素材包。
...
| 阶段 | 调研报告目标 | 当前状态 | 证据 |
|---|---|---|---|
| P0 | 修复依赖和文档一致性，统一策略、结果、图表和报告口径 | 已完成 | `outputs/results/*.csv`、`report/实验报告.md`、`ppt/notebooklm/upload_pack/` 已同步七模型/七链路口径；离线表新增 RepeatRecall/ExploreRecall |
| P1 | 新增批量最大权骑手匹配，并与逐单贪心对比 | 已完成 | `foodflow/rider_sim.py` 中容量槽位匹配；结果同时包含 `Seq-xQuAD-Tripartite + Greedy` 和 `Seq-xQuAD-Tripartite + Batch` |
...
- `Session-SPU-Tripartite`，只使用训练期会话和 SPU 信号
