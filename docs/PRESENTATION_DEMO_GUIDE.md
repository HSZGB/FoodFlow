...
7. 看空间图，说明用户、商家、骑手候选和取餐/配送路径。
8. 看骑手候选榜，讲为什么不是简单最近，而是 ETA、负载、可靠性综合。
9. 切换 `Seq-Tuned` 或 `UserOnly`，对比推荐理由和 ETA 变化；若被问知识图谱，切 `KG-Tripartite`，讲推荐理由里的品类/区域/价位图谱路径（深度版模型在 kg-demo 子项目）。
10. 打开午餐高峰回放，讲多时间步下骑手状态会持续更新。
