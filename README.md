...
| KG-Tripartite + RouteMinETA | 36.15 ±0.96 | 0.229 ±0.032 | 0.542 ±0.011 |
| KG-Tripartite + RouteBatch | 38.84 ±1.24 | 0.311 ±0.041 | 0.517 ±0.014 |

**运力紧张压力场景（30 骑手，模拟高峰爆单，`make simulate-stress`）**下，路径感知顺路派单大幅反超所有槽位匹配策略——顺路接单率达 66.9%，平均 ETA 比最好的槽位匹配低约 16 分钟（远超置信区间）：
...
| KG-Tripartite + RouteMinETA | **44.88 ±1.55** | **0.457 ±0.038** | **0.544 ±0.016** |
| KG-Tripartite + RouteBatch | 47.34 ±1.66 | 0.515 ±0.033 | 0.525 ±0.015 |

结论：`Seq-Tuned` 取得最强离线准确性；三方重排 + 批量匹配在常规运力下系统性改善履约；**顺路派单在常规运力下与最优槽位匹配统计打平（ETA 置信区间重叠），在高峰运力吃紧时大幅胜出**——route 家族的 ETA 诚实计入队列沿路径行进的时间，仍能做到这一点，说明合单的收益是真实的。派单机制可随供需状态切换，这是三方联合视角的核心论据。
