...
4. 离线推荐：默认评估 Popular、BPR-MF、UserOnly、Seq-Tuned、LightGBM/Logistic-LTR、Seq-xQuAD-Tripartite、Session-SPU-Tripartite 七个代表策略。
5. 三方重排：在用户偏好外加入列表级覆盖、商家公平、ETA 和供给分。
6. 动态履约仿真：默认比较三条基础链路、学习排序、三方逐单/批量匹配和 Session-SPU 增强共七条链路。
...
- `Session-SPU-Tripartite`：只使用训练期点击会话和菜品 SPU 画像扩展候选与解释，避免测试期信息泄漏。
...
- `Seq-xQuAD-Tripartite + Batch`：列表级三方重排 + 容量槽位批量最大权匹配。
- `Session-SPU-Tripartite + Greedy`：训练期会话/SPU 增强 + 负载感知逐单派单。
...
- `Session-SPU-Tripartite` Recall@20 = `0.4147`，NDCG@20 = `0.3431`；它不追求离线单项最优，而是为系统级仿真提供训练期会话与菜品信号。
...
- 批量三方匹配在相同订单上降低 ETA 和超时率。
- `Session-SPU-Tripartite + Greedy` 的平均 ETA、超时率和平台综合效用当前最优。
