...
生成 11 页 PPT，用于数据挖掘/推荐系统课程大作业展示。PPT 必须覆盖：
...
- Codex PPT 工作流要求：先确认大纲、视觉风格、图片后端和 1 页样张，再生成全套图片页、讲稿和 `.pptx`；在这些门禁完成前，不创建最终 `deck_spec.json`、`speech.md`、整套图片页或 `.pptx`。
- Codex PPT 输出默认为统一风格的整页图片式幻灯片；每页图片必须来自确认过的图片后端，并保留同一视觉身份。
- Image to Editable PPT 不用于从零创作本 PPT；它只用于后续把已有图片页、PDF、截图或图片式 PPT 重建成对象级可编辑 `.pptx`。
- 如果后续需要对象级可编辑化，应在已有视觉页之后再走 `editppt prepare -> page worker -> record -> finalize` 流程；不要承诺 NotebookLM 一步生成的页面天然满足对象级可编辑。
...
- `Popular + Nearest`：Avg ETA = `86.64`，Timeout Rate = `0.8537`，Platform Utility = `0.3098`。
...
5. **推荐算法与三方重排**  
   对比 Popular、BPR-MF、UserOnly、Seq-Tuned、LightGBM-LTR、Seq-xQuAD-Tripartite；展示 Seq-xQuAD-Tripartite 的三方重排拆解。
...
如果你可以直接生成 PPT，请生成可下载的 PPT 文件。如果不能直接生成 PPT，请输出可以复制到 PPT 工具中的逐页内容；这份逐页内容应方便后续交给 Codex PPT 或其他工具制作图片页，也方便在已有图片页之后再做可编辑化重建。每页包含：
